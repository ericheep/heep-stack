    ...
    if (cols > 0) {
      ...
      for (int i = 0; i < cols; i++) {
        if (i % 2 == 0) {
          ...
          if (i % 2 == 0) {
            if (rows == 1) {
              line(row_x1 + (i * s), y - (0.25 * s) - ex_mod, row_x1 + (i * s), 0.25 * s + y + ex_mod);
            } else {
              line(row_x1 + (i * s), col_y1 - ex_mod, row_x1 + (i * s), col_y2 + ex_mod);
            }
          }
        }
      }
      // metro
      for (int i = 0; i < cols * 4.0; i++) {
        if (p % 2 == side && r == i && i % 4 != 0) {
          line(row_x1 + (i * s * 0.5), y + 5, row_x1 + (i * s * 0.5), y - 5);
        }
      }
    }
